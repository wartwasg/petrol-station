...
// Get assigned pump
$assignedPump = $pdo->prepare("
    SELECT p.*, ft.name as fuel_type
    FROM pumps p
    JOIN fuel_types ft ON p.fuel_type_id = ft.id
    WHERE p.attendant_id = ?
");
$assignedPump->execute([$userId]);
$pump = $assignedPump->fetch();

// Get today's readings for assigned pump
$today = date('Y-m-d');
$todayReadings = [];
if ($pump) {
    $readingsStmt = $pdo->prepare("
        SELECT * FROM pump_readings 
        WHERE pump_id = ? AND reading_date = ?
    ");
    $readingsStmt->execute([$pump['id'], $today]);
    $todayReadings = $readingsStmt->fetchAll();
}
...
<head>
    ...
    <title>Pump Attendant Dashboard - Petrol Station</title>
    ...
    <style>
        ...
        .pump-card { border: none; border-radius: 15px; box-shadow: 0 5px 20px rgba(0,0,0,0.08); }
        ...
        @media (max-width: 768px) {
            ...
            .pump-card { padding: 15px 10px; }
            .pump-card h3 { font-size: 1.3rem; }
            .pump-card h4 { font-size: 1.1rem; }
            .pump-card h5 { font-size: 1rem; }
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        <div class="text-center py-4">
            ...
            <small class="text-white-50">Pump Attendant</small>
        </div>
        <ul class="nav flex-column">
            <li class="nav-item"><a href="dashboard.php" class="nav-link active" data-bs-toggle="tab"><i class="fas fa-home"></i> Dashboard</a></li>
            <li class="nav-item"><a href="#pump" class="nav-link" data-bs-toggle="tab"><i class="fas fa-gas-pump"></i> My Pump</a></li>
    ...
    <div class="main-content">
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane fade show active" id="dashboard">
                    ...
                    <div class="col-md-4">
                        <div class="card stat-card p-3 text-white bg-primary">
                            <div class="d-flex justify-content-between">
                                <div>
                                    <small>Status</small>
                                    <h4 class="mb-0"><?php echo $pump ? 'Assigned' : 'Not Assigned'; ?></h4>
                                </div>
                    ...
                    <div class="col-md-4">
                        <div class="card stat-card p-3 text-white bg-info">
                            <div class="d-flex justify-content-between">
                                <div>
                                    <small>Role</small>
                                    <h4 class="mb-0">Pump Attendant</h4>
                                </div>
                ...
                <?php if ($pump): ?>
                <div class="card pump-card p-4">
                    <div class="text-center">
                        <i class="fas fa-gas-pump fa-4x text-primary mb-3"></i>
                        <h3><?php echo htmlspecialchars($pump['pump_number']); ?></h3>
                        <p class="text-muted"><?php echo htmlspecialchars($pump['fuel_type']); ?></p>
                        <span class="badge bg-<?php echo $pump['is_active'] ? 'success' : 'danger'; ?> fs-6">
                            <?php echo $pump['is_active'] ? 'Active' : 'Inactive'; ?>
                        </span>
                    </div>
                </div>
                <?php else: ?>
                <div class="card pump-card p-4">
                    <div class="text-center">
                        ...
                        <h3>No Pump Assigned</h3>
                        <p class="text-muted">Please contact your manager to assign you to a pump.</p>
                    </div>
                </div>
                <?php endif; ?>
            </div>
            ...
            <!-- My Pump -->
            <div class="tab-pane fade" id="pump">
                <?php if ($pump): ?>
                <div class="card pump-card p-4">
                    <h4 class="mb-4"><i class="fas fa-gas-pump me-2"></i>Pump Details</h4>
                    <div class="row">
                        <div class="col-md-6">
                            <table class="table table-borderless">
                                <tr>
                                    <td><strong>Pump Number:</strong></td>
                    ...
                    <h5 class="mt-4"><i class="fas fa-clock me-2"></i>Today's Readings</h5>
                    <?php if (empty($todayReadings)): ?>
                        <div class="alert alert-warning">
                            <i class="fas fa-info-circle me-2"></i>No readings recorded for today. Please wait for the manager to record readings.
                        </div>
                    <?php else: ?>
                ...
                <?php else: ?>
                <div class="alert alert-warning">
                    <i class="fas fa-exclamation-triangle me-2"></i>You are not assigned to any pump yet.
                </div>
                <?php endif; ?>
            </div>
                        ...
                        <div class="card pump-card p-4">
                            ...
                            <form method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" value="Pump Attendant" disabled>
                                </div>
                                <div class="mb-3">
